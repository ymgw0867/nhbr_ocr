...
namespace NHBR_OCR.common
{
    class OCRData
    {
        ...
        private void setNewHeadRecRow(ref NHBR_CLIDataSet.FAX注文書Row r, string[] stCSV)
        {
            ...
            r.パターンID = Utility.StrtoInt(Utility.GetStringSubMax(stCSV[2].Trim(), 4));
            r.届先番号 = Utility.StrtoInt(Utility.GetStringSubMax(stCSV[3].Trim(), 6));
            r.発注番号 = Utility.GetStringSubMax(stCSV[5].Trim(), 8);
            r.納品希望月 = Utility.GetStringSubMax(stCSV[6].Trim(), 2);
            r.納品希望日 = Utility.GetStringSubMax(stCSV[7].Trim(), 2);
            r.確認 = global.flgOff;
        ...
        private void setNewItemRecRow(ref NHBR_CLIDataSet.FAX注文書Row r, string[] stCSV, int iX, string mCode)
        {
            ...
            if (iX == 10)
            {
                r.注文数10 = stCSV[0].Trim();
            }

            if (iX == 11)
            {
                r.注文数11 = stCSV[0].Trim();
            }

            if (iX == 12)
            {
                r.注文数12 = stCSV[0].Trim();
            }

            if (iX == 13)
            {
                r.注文数13 = stCSV[0].Trim();
            }

            if (iX == 14)
            {
                r.注文数14 = stCSV[0].Trim();
            }

            if (iX == 15)
            {
                r.注文数15 = stCSV[0].Trim();
            }

            if (iX == 16)
            {
                r.注文数16 = stCSV[0].Trim();
            }

            if (iX == 17)
            {
                r.注文数17 = stCSV[0].Trim();
            }

            if (iX == 18)
            {
                r.注文数18 = stCSV[0].Trim();
            }

            if (iX == 19)
            {
                r.注文数19 = stCSV[0].Trim();
            }

            if (iX == 20)
            {
                r.注文数20 = stCSV[0].Trim();
            }

            if (iX == 21)
            {
                r.注文数21 = stCSV[0].Trim();
            }

            if (iX == 22)
            {
                r.注文数22 = stCSV[0].Trim();
            }

            if (iX == 23)
            {
                r.注文数23 = stCSV[0].Trim();
            }

            if (iX == 24)
            {
                r.注文数24 = stCSV[0].Trim();
            }

            if (iX == 25)
            {
                r.注文数25 = stCSV[0].Trim();
            }

            if (iX == 26)
            {
                r.注文数26 = stCSV[0].Trim();
            }

            if (iX == 27)
            {
                r.注文数27 = stCSV[0].Trim();
            }

            if (iX == 28)
            {
                r.注文数28 = stCSV[0].Trim();
            }

            if (iX == 29)
            {
                r.注文数29 = stCSV[0].Trim();
            }

            if (iX == 30)
            {
                r.注文数30 = stCSV[0].Trim();
            }

            if (iX == 31)
            {
                r.追加注文数1 = stCSV[0].Trim();
            }

            if (iX == 32)
            {
                r.追加注文数2 = stCSV[0].Trim();
            }

            if (iX == 33)
            {
                r.追加注文数3 = stCSV[0].Trim();
            }

            if (iX == 34)
            {
                r.追加注文数4 = stCSV[0].Trim();
            }

            if (iX == 35)
            {
                r.追加注文数5 = stCSV[0].Trim();
            }

            if (iX == 36)
            {
                r.追加注文数6 = stCSV[0].Trim();
            }

            if (iX == 37)
            {
                r.追加注文数7 = stCSV[0].Trim();
            }

            if (iX == 38)
            {
                r.追加注文数8 = stCSV[0].Trim();
            }

            if (iX == 39)
            {
                r.追加注文数9 = stCSV[0].Trim();
            }

            if (iX == 40)
            {
                r.追加注文数10 = stCSV[0].Trim();
            }
