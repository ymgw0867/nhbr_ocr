...
namespace NHBR_OCR.OCR
{
    public partial class frmNgRecovery : Form
    {
        ...
        string _camRec = Properties.Settings.Default.campaignPath;
        ...
        ///--------------------------------------------------------------
        /// <summary>
        ///     キャンペーンリカバリフォルダへ指定画像を移動する </summary>
        ///--------------------------------------------------------------
        private void toCamDir()
        {
            try
            {
                if (ngFileCount() == 0)
                ...
                {
                    if (MessageBox.Show(ngFileCount().ToString() + "件の画像をキャンペーンリカバリーフォルダへ移動します。よろしいですか？", "確認", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                        return;
                }

                // 出力先フォルダ
                if (!System.IO.Directory.Exists(Properties.Settings.Default.campaignPath))
                {
                    System.IO.Directory.CreateDirectory(Properties.Settings.Default.campaignPath);
                }
                ...
                for (int i = 0; i < checkedListBox1.Items.Count; i++)
                {
                    if (checkedListBox1.GetItemChecked(i))
                    {
                        File.Move(ngf[i].ngFileName, _camRec + System.IO.Path.GetFileName(ngf[i].ngFileName));
                    }
                }

                // 終了メッセージ
                MessageBox.Show(ngFileCount().ToString() + "件の画像をローカルＰＣのキャンペーンリカバリーフォルダへ移動しました", "処理完了", MessageBoxButtons.OK, MessageBoxIcon.Information);
